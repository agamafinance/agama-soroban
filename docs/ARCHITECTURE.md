...
| `register_pool(admin, pool_id, originator, jurisdiction, cap_bps)` | Whitelists a pool with metadata and cap. Refuses any adapter that does not name this Engine and this Engine's Vault: an adapter pointed elsewhere takes capital from this Vault and repays a third party while `deallocate` decrements the book as though the money had come home. This is the first time that check runs rather than the only time — half of the condition is a fact about the Engine, and `set_vault` can change it, so `allocate`, `deallocate` and `recover` each re-run it on the pool they touch. |
