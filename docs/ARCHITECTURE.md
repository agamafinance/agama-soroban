...
**The one money movement with no event on the custody side.** Every call that moves USDC in or out of the Vault emits, and the events are specific: `deposit`, `request_withdrawal`, the payout behind `claim_withdrawal` and `settle_withdrawal`, `record_repayment`, `record_writedown`, `record_recovery`. The exception is `settle_allocation`, which is the call that sends USDC out to a pool, and it emits nothing. Both pool adapters have the same shape: `write_down`, `recover_surplus` and `set_counterparties` emit, while `allocate` and `deallocate` do not.

So the only on-chain event recording an allocation is `Engine::Allocated`, and it is a good one, carrying the pool, the amount, the pool's exposure afterwards and the idle reserves left. The gap is not what is recorded but who attests to it. This design distrusts the Engine on purpose: the Vault keeps its own `reserve_floor_bps` and its own `deployed_capital` book, and re-checks the floor in `settle_allocation`, precisely because a floor enforced only in the Engine is a floor any contract holding that authorization can skip. The audit trail does not carry that distrust through. An Engine that released capital within the floor and emitted nothing, or emitted a number that did not match what it asked the Vault for, would leave the Vault's side visible only to somebody polling `deployed_capital` between two ledgers.

The direction is also the wrong way round for monitoring. Money arriving is announced three different ways; money leaving for a pool is silent, and outflow is the half worth watching.

Not fixed, and the cost is the reason. Emitting from `settle_allocation` means a Vault generation, which costs three contracts and strands the recovered principal described above. Emitting from the adapters is cheaper, since both are empty and `unregister_pool` accepts a pool carrying neither exposure nor written-off charge, but it fixes the half that matters less: an adapter event still fires on the Engine's call, and the custody side stays silent. Recorded here so that it is met as a known limit rather than found.

**Where the Vault's own equity can go, which is nowhere.** USDC leaves this Vault through exactly two calls. `settle_allocation` sends it to a registered pool adapter, which is bounded by the floor and by the caps and whose only outbound path sends it back to the Vault that adapter stores. The payout helper behind `claim_withdrawal` and `settle_withdrawal` pays a queued claim, which is bounded by the agUSD that was burned to create it. There is no third.

So a Vault holding more USDC than agUSD claims cannot pay the difference to anybody, including its admin. On testnet that difference is real and large: recovering capital through `recover` or `book_recovery` books cash without minting agUSD against it, which is correct, because recovered principal is the protocol's and not a depositor's. The book ends over-collateralised and stays that way.

This is a property rather than a gap, and the absence is the point: a Vault with an entry point that pays equity to an admin is a Vault an admin can empty, and no guard on such a call is worth as much as not having it. The cost is that equity accumulated this way is permanent, and that a Vault generation carrying it strands it, since a replacement Vault has no way to take it either. That is a real number to weigh before deciding a redeployment is cheap.
