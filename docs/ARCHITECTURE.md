...
| `floor_base() → i128` | The denominator **this Engine** measures against: the Vault's `free_reserves()`, plus what this Engine has booked as deployed, plus `written_off()`. Not the same quantity as `Vault::floor_base()`, which is measured on accounted cash. This one is higher by any USDC that reached the Vault without its books being told, so it is the more permissive of the two and the Vault's is the one that binds. See 6.1. |
| `get_reserve_ratio() → u32` | Free reserves as an actual share of this Engine's base, in bps: the number the floor is a lower bound on, read against the same base the Engine checks. Both sides read the raw balance net of the queue, so unaccounted cash lifts this reading above what the Vault would allow. Measured against net assets instead, this went *up* when a loss was recognised, which is the opposite of what losing money does to a liquidity position. |
...
**The Engine was not moved with it, and that is a divergence rather than a decision.** `Engine::floor_base()`
and `Engine::get_reserve_ratio()` still build their base from `Vault::free_reserves()`, which reads the real token
balance. So for any unaccounted cash `U` the Engine's base is higher by `U` and its free-reserves term is higher by
`U` too, and since the floor asks for a fraction of the base, the Engine comes out more permissive by
`(1 - floor_bps) * U`. The direction is the safe one: `settle_allocation` re-checks on accounted cash in the same
transaction, so an allocation the Engine would allow and the Vault would not reverts with `InsufficientLiquidity` or
`ReserveFloorBreached` and moves nothing. What it does cost is a public readout that overstates: `get_reserve_ratio()`
is the number an integrator reads, and it counts cash the Vault will not deploy. Open, and listed as such.
